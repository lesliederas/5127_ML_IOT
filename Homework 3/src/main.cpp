...
void setup()
{
  ...
  model = tflite::GetModel(sine_predictort_tensorflowlite);
...
void loop()
{
  ...
  if (chars_avail > 0)
  {
    ...
    { // 13 decimal = newline character
      ...
      {
        Serial.println("Error please enter 7 numbers ");
      }
      else
      {
        for (int x = 0; x < array_length; x++)
        {
          input->data.int8[x] = static_cast<int8>(input_array[x]);
        }
        ...
        int8_t predict = output->data.int8[0];
